...
# how many bites
df_eda %>% count(bite_flag, sort = TRUE)
...
# merge FALSE and NA for bite_flag
df_eda <- df %>% mutate(
  incident_present = 
    !is.na(child_id) |
    !is.na(child_age_at_bite) |
  ...
  df_eda %>% count(bite_flag_correct)
  
df_eda %>% count(bite_flag, sort = TRUE)
df_eda %>% count(bite_flag_correct, sort = TRUE)
df_eda %>% count(has_child_been_bitten, bite_flag_correct)
```
